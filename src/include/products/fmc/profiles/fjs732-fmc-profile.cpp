...
// The FlyJSim 737-200 drives the stock Universal Avionics UNS-1 module, so the
// CDU display lives under "uns1/cdu1" and the FMS keys under "uns1/fms1".
static const std::string kCdu = "uns1/cdu1";
static const std::string kFms = "uns1/fms1";
static constexpr int kScreenLines = 11;

FJS732FMCProfile::FJS732FMCProfile(ProductFMC *product) : FMCAircraftProfile(product) {
...
const std::vector<std::string> &FJS732FMCProfile::displayDatarefs() const {
...
const std::vector<FMCButtonDef> &FJS732FMCProfile::buttonDefs() const {
    static const std::vector<FMCButtonDef> buttons = {
        ...
        {FMCKey::PLUSMINUS, kFms + "/plusminus", FMCDatarefType::EXECUTE_CMD_ONCE},
...
const std::unordered_map<FMCKey, const FMCButtonDef *> &FJS732FMCProfile::buttonKeyMap() const {
...
void FJS732FMCProfile::updatePage(std::vector<std::vector<char>> &page) {
    UNS1Decode::updatePage(product, page, kCdu, kScreenLines);
}

void FJS732FMCProfile::buttonPressed(const FMCButtonDef *button, XPLMCommandPhase phase) {
